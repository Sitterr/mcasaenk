    ...
    public static class ColormapMaker {
        ...
        public static void FromResourcePacks(string path, string[] resourcepacks, int f, double q = 1) {
...

            {
                ...
                List<string> lines = new() { "// automatically generated by Utils.AssetsUtils.CreateResourcepackColormap" };
                foreach(var r in res) {
                    string name = r.Key;
                    if(name.StartsWith("minecraft:")) name = name.Substring(10);
                    string color = r.Value;

                    lines.Add($"{name};{color}");
                }
...

            {
                ...
                TxtFormatReader.ReadStandartFormat(Resources.tintblocks, (group, parts) => {
                    ...
                    if(tints.ContainsKey(name)) {
                        tints[name].blocks.AddRange(parts[2].Split(',').Select(x => x.Trim()).ToList());
                    } else {
            ...
            Dictionary<string, string> ModelsToMeanColor(string[] resourcepacks, Dictionary<string, string> blocksstatemodel, double q) {
                ...
                foreach(var resourcepack in resourcepacks.Reverse()) {
                    foreach(var blockmodel in blocksstatemodel) {
                        ...
                        string color = ReadMeanColorOfModel(resourcepack, blockmodel.Value, q);
                        if(color != "") {
                            map.Add(blockmodel.Key, color);
                        }
                    }
                }
                return map;
            }
            string ReadMeanColorOfModel(string resource_pack, string modellocation, double q) {
                ...
                string texturePath = Path.Combine(resource_pack, toLocation(texturename, "textures", "png"));
                if(Path.Exists(texturePath) == false) return "";
        ...
        public static void FromJavaMap(string path, string resourcepack, Bitmap blockMap) {
            ...
            createFileFromLines(archive, "__colormap__", map.Select(v => $"{v.Key};{v.Value}").ToArray());
        }
        ...
        public static void FromBedrockMap(string path, string resourcepack, Bitmap blockMap, Bitmap[] biomeMaps) {
            ...
            createFileFromLines(archive, "grass.properties", ["format=grid", $"blocks={string.Join(" ", tints.First(t => t.tint == "grass").blocks)}", "source=grass.png"]);
            createFile(archive, "grass.png", ImageToByte2(newGrassColors));

            createFileFromLines(archive, "foliage.properties", ["format=grid", $"blocks={string.Join(" ", tints.First(t => t.tint == "foliage").blocks)}", "source=foliage.png"]);
            createFile(archive, "foliage.png", ImageToByte2(newFoliageColors));

            createFileFromLines(archive, "water.properties", ["format=grid", $"blocks={string.Join(" ", tints.First(t => t.tint == "water").blocks)}", "source=water.png"]);
